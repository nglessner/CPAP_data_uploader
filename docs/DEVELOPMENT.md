...
- **SMB path layout:** files land at `<ENDPOINT>/<O2RING_PATH>/<deviceSegment>/<filename>.vld`. The server-side staging pipeline requires the per-device subdir. `deviceSegment` is resolved once at `Config::loadFromSD` — sanitized `DEVICE_NAME` from `config.txt` if set, else the WiFi MAC (colons stripped, lowercased). Don't write directly into `<O2RING_PATH>` — those uploads are rejected.
